...
constexpr unsigned int N_PASSWORDS_TO_CRACK = 19;
...
// passwords to crack
char passwords[N_PASSWORDS_TO_CRACK][41] = {
...
const unsigned int hash_length = gcry_md_get_algo_dlen(GCRY_MD_SHA1);
unsigned char binary_passwords[N_PASSWORDS_TO_CRACK][20];
char alphabet[256];
chrono::time_point<chrono::system_clock> start;

// converts hex passwords to binary format
void create_binary_passwords() {
    for (unsigned i = 0; i < N_PASSWORDS_TO_CRACK; ++i) {
        for(unsigned j = 0; j < hash_length; j++) {
            char a = passwords[i][j * 2];
            char b = passwords[i][j * 2 + 1];
            if (a <= '9') a = a - '0'; else a = a - 'a' + 10;
            if (b <= '9') b = b - '0'; else b = b - 'a' + 10;
            binary_passwords[i][j] = a * 16 + b;
        }
    }
}

bool check_pwd(const unsigned char* pwd) {
    for (unsigned i = 0; i < N_PASSWORDS_TO_CRACK; ++i) {
        if (memcmp(binary_passwords[i], pwd, hash_length) == 0)
            return true;
    }
    return false;
}

void dumpHash(const unsigned char* hash) {
...
void pwd_thread(
        char* p,
        const unsigned int alphabet_length,
        const unsigned int idx,
        const unsigned int idx_end,
        const int d,
        const int digit) {
    if (d > 1) {
        for (unsigned b = 0; b < alphabet_length; ++b) {
            p[d - 1] = alphabet[b];
            pwd_thread(p, alphabet_length, idx, idx_end, d - 1, digit);
        }
    } else {
        for (unsigned i = idx; i < idx_end && i < alphabet_length; ++i) {
            p[0] = alphabet[i];
...
// alphabet = 'abc' // used alphabet, possible characters in the password
...
//                2nd and 3rd characters of the password: a | b | c
void generate_pwd_multi_thread(const unsigned int n_threads, const int digit) {

    // calculates how many characters are for one thread
    int length = strlen(alphabet) / n_threads;
    if ((strlen(alphabet) % n_threads) > 0) {
        length++;
    }
    ...
    for (unsigned t = 0; t < n_threads; ++t) {
        ...
        if (ps != nullptr) {
            ...
            threads[t] = thread(pwd_thread, p, strlen(alphabet), idx, idx + length,
                    digit, digit);
    ...
    auto duration = chrono::duration_cast<chrono::seconds>(chrono::system_clock::now() - start);
    printf("%d char: %ld sec\n", digit, duration.count());
}
...
int main() {
    ...
    create_binary_passwords();

    // build allowed character set
    strcpy(alphabet, alphabet1);
    strcat(alphabet, alphabet2);
    // strcat(alphabet, alphabet3);
    // strcat(alphabet, alphabet4);
    printf("alphabet length: %lu\n", strlen(alphabet));
    ...
    for (unsigned i = MIN_DIGITS; i <= MAX_DIGITS; ++i) {
        threads[i] = thread(generate_pwd_multi_thread, n_threads, i);
        this_thread::sleep_for(chrono::milliseconds(1000));
    }
    for (unsigned i = MIN_DIGITS; i <= MAX_DIGITS; ++i) {
        threads[i].join();
